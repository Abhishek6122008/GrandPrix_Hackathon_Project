...
def _to_advisory(text: str | Advisory, request: AnalyzeRequest) -> Advisory:
    """
    ...
    The offline path in `app.scoring` builds an `Advisory` directly and passes it straight
    through: it is already structured, and parsing it back out of a rendered string would only
    lose the headline and actions it took care to separate.
    """
    if isinstance(text, Advisory):
        return text
