...
async def lifespan(app: FastAPI):
    """One line at boot saying which inference path is actually live. Saves an hour of
    'why is the advisory generic' during a demo. Nothing is loaded — see the module docstring."""
    config = settings.describe()
    log.info(
        "AI service ready — GNN: %s | advisory: %s | local fallback: %s",
        "hugging face" if config["hostedGnn"] else scoring.MODEL_NAME,
...
def health() -> dict:
    ...
    """
    config = settings.describe()
    usable = config["localFallback"] or (config["hostedGnn"] and config["hostedLlm"])
    return {
        "status": "ok" if usable else "degraded",
        "service": "crowdflow-ai",
        "inference": {
            "gnn": "huggingface" if config["hostedGnn"] else scoring.MODEL_NAME,
